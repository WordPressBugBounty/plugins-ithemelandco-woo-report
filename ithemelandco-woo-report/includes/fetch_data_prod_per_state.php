<?php
	...
	if($file_used=="sql_table")
	{
		...
		$sql = "SELECT $sql_columns
			FROM $sql_joins $category_id_join $brand_id_join $all_tax_joins $it_id_order_status_join
			WHERE $sql_condition  $it_from_date_condition $category_id_condition $brand_id_condition $all_tax_conditions
			$it_cat_prod_id_string_condition $it_brand_prod_id_string_condition $it_id_order_status_condition $it_product_id_condition $it_country_code_condition $state_code_condition $it_order_status_condition $it_hide_os_condition
		...
		}else if($it_country_code && $it_country_code!='-1') //IF STATE NOT SELECTED AND COUNTRY SELETED
		{
			$country_states = $this->it_get_woo_country_of_state();
			$state_codes_per_country=array();
			$it_country_code		= explode(",",$this->it_get_woo_requests('it_countries_code','-1',true));
			if($it_country_code && $it_country_code!='-1')
			{
				$index=0;
				foreach($country_states as $country_states_val){
					if(in_array($country_states_val->parent_id,$it_country_code)){
		...
		$value=array(array('lable'=>esc_html__('Total','ithemelandco-woo-report'),'status'=>'currency'));
		array_splice($this->table_cols, $array_index, 0, $value );
		...
		//for($i=1; $i<=20 ; $i++){
			$datatable_value.=("<tr>");
				...
				//print_r($arr);
				$items_product=$this->it_get_woo_cp_items($type , $items_only, $id,$params,'_billing_state');
				...
				foreach($this->data_state as $state_name){
					...
					if(isset($state_arr[$state_name]['total'])){
						$it_table_value=$this->price($state_arr[$state_name]['total']) .' #'.$state_arr[$state_name]['qty'];
